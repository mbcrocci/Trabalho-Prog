...
Configuracoes Choose_Config(int *PrimVez){	//Escolher qual a configuraÁ„o a utilizar na simulaÁ„o 
	...
	do{
		...
		if (r == 1){ C = Read_Config("config1.txt", &falha); }
		else{
			if (r == 2){ C = Read_Config("config2.txt", &falha); }
			else{
				if (r == 3){ C = Read_Config("config3.txt", &falha); }
				else { C = Read_Config("configpes.txt", &falha); }
			}
		}
		if (falha == 1){
...
int MenuConfig(){	//menu das opÁoes que o utilizador pode escolher
	...
	do{
		linha();
		puts("1 - Visualizar Configuracoes Pre-definidas");
		puts("2 - Escolher Configuracoes Pre-definidas");
